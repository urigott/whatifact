...
class TestPreprocessing(unittest.TestCase):
    ...
    def test_get_variables_and_widgets(self):
        ...
        self.assertTrue(all(['options' in v.keys() for v in variables.values() if v['type'] == 'categorical']))
        self.assertTrue(all(['value' in v.keys() for v in variables.values() if v['type'] == 'continuous']))
