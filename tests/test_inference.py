...
class TestAssertions(unittest.TestCase):
        ...
        self.df = pd.DataFrame(
            {
                ...
                'col3': [i for i in range(10)] * 3, # categorical, will be defined below
    ...
    def test_assert_features_fail_overlap(self):
        with self.assertRaises(AssertionError) as e:
            _ = _assert_features(df=self.df, 
                                categorical_features=self.categorical_features,
                                continuous_features=self.continuous_features + [self.categorical_features[0]]
                                )
    ...
    def test_assert_features_fail_unavailable_continuous_features(self):
        ...
        self.assertIn("Some of continuous_features are not in dataframe", str(e.exception))
    ...
    def test_assert_features_fail_unavailable_categorical_features(self):
        ...
        self.assertIn("Some of categorical_features are not in dataframe", str(e.exception))
            
    def test_assert_features_fail_wrong_continuous_features(self):        
        with self.assertRaises(AssertionError) as e:
            _ = _assert_features(df=self.df, 
                                categorical_features=self.categorical_features[:2],
                                continuous_features=self.continuous_features + [self.categorical_features[-1]]
                                )
        self.assertIn("continuous_features include non-numeric columns", str(e.exception))
            
    def test_assert_features_fail_wrong_categorical_features(self):        
        with self.assertRaises(AssertionError) as e:
            _ = _assert_features(df=self.df, 
                                categorical_features=self.categorical_features + [self.continuous_features[-1]],
                                continuous_features=self.continuous_features[:1]
                                )
        self.assertIn("categorical_features include non-categorical features", str(e.exception))

    def test_assert_features_drop_columns(self):        
        df_redundant = self.df.copy().assign(col6 = np.arange(30))
        
        df_asserted = _assert_features(df_redundant,
                                      categorical_features=self.categorical_features,
                                      continuous_features=self.continuous_features
                                      )
    ...
    def test_assert_feature_settings_pass(self):
        ...
        _assert_feature_settings(df=self.df,
