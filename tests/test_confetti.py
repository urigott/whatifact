...
class TestConfetti(unittest.TestCase):
    def setUp(self):
        ...
        self.df = pd.DataFrame({
            ...
            'Sex': pd.Series(np.random.choice(["M", "F"], size=n, replace=True)).astype('category'),
            'Hb': pd.Series(np.random.normal(loc=12, size=n)),
            'Diabetic': pd.Series(np.random.binomial(n=2, p=0.25, size=n)).astype('category')
        })
        ...
        self.df_numerical['Sex'] = self.df_numerical['Sex'].map({'M': 0, 'F': 1}).astype(int)
    ...
    def test_with_lgbm(self):
        clf = lgb.LGBMClassifier(verbose=-1).fit(self.df.drop('sample_id', axis=1), self.labels)
        _ = confetti(df=self.df,
                     clf=clf,
                     sample_id='sample_id',
    ...
    def test_with_lgbm_with_missing_values(self):        
        ...
        df_missing['sample_id'] = self.df['sample_id'] # sample_id should not have missing values
        df_missing = df_missing.astype({'Age': float, 'Height': float, 'Sex': 'category', 'Hb': float, 'Diabetic': 'category'})

        self.assertGreater(df_missing.isna().sum().sum(), 0) # assert df_missing really has missing values

        clf = lgb.LGBMClassifier(verbose=-1).fit(df_missing.drop('sample_id', axis=1), self.labels)
        _ = confetti(df=df_missing,
                     clf=clf,
                     sample_id='sample_id',
                     continuous_features=self.continuous_features,
                     categorical_features=self.categorical_features,
    ...
    def test_with_logistic_regression(self):
        ...
        clf = LogisticRegression(verbose=0).fit(self.df_numerical.drop('sample_id', axis=1), self.labels)
        _ = confetti(df=self.df_numerical,
                     clf=clf,
                     sample_id='sample_id',
                     continuous_features=self.continuous_features,
                     categorical_features=self.categorical_features,
