...
from src.widgets import _get_drop_down, _get_single_slider, _set_null_in_variable, _un_null_variable
from src.inference import _get_sliders_params, _get_select_list_params



def _get_variables_and_widgets(df, 
    ...
    for j, col in enumerate(df.columns, start=1):
        ...
        if col in continuous_features:            
            var_dict.update(**_get_sliders_params(df[col], 
                                                 col_dict=feature_settings.get(col, {})))
            
        elif col in categorical_features:
            var_dict.update(**_get_select_list_params(df[col], 
                                                     col_dict=feature_settings.get(col, {})))                        
            

        variables.update({var_id: var_dict})
        widgets.update({var_id: _get_single_slider(v=var_dict) if var_dict['type'] == 'continuous' else _get_drop_down(v=var_dict)})        
...
def _update_values(df, sample, variables):            
    ...
    for var_id, v in variables.items():
        ...
        if v['type'] == 'categorical':
            ui.update_select(id=v['id'], selected=str(new_value) if not pd.isna(new_value) else '')
