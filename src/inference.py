...
def _prepare_everything(df: pd.DataFrame, 
                        clf: object,
                        sample_id: str,
...
def _handle_sample_id(df: pd.DataFrame, sample_id: str = None) -> Tuple[pd.DataFrame, Callable, bool]:
    if sample_id:        
...
def _assert_features(df: pd.DataFrame, 
                     continuous_features: list, 
                     categorical_features: list) -> pd.DataFrame:
    
    # assertions
    assert set(continuous_features).intersection(categorical_features) == set(), f"Continuous and categorical features overlap: {list(set(continuous_features).intersection(categorical_features))}"
    assert set(continuous_features) - set(df.columns) == set(), f"Some of continuous_features are not in dataframe: {list(set(continuous_features) - set(df.columns))}"
    assert set(categorical_features) - set(df.columns) == set(), f"Some of categorical_features are not in dataframe: {list(set(categorical_features) - set(df.columns))}"
    assert all(pd.api.types.is_numeric_dtype(df[col]) for col in continuous_features), "continuous_features include non-numeric columns"
    assert all(df[col].dtype == 'object' or df[col].dtype.name == 'category' or df[col].nunique() <= 10 for col in categorical_features), "categorical_features include non-categorical features (i.e., of type 'object', 'category', or have up to 10 unique values)"
    
    # remove columns from data frame that are not required
    redundant_features = set(df.columns) - set(continuous_features) - set(categorical_features)    
    df.drop(list(redundant_features), axis=1, inplace=True)

    return df

def _assert_feature_settings(df: pd.DataFrame, 
    ...
    for k, v in feature_settings.items():
        ...
        if k in df.columns and k in continuous_features:
            if 'min' in v.keys():
                assert v['min'] <= arr.min(), f"'min' slider value smaller than minimal value in {k}"
            if 'max' in v.keys():
                assert v['max'] >= arr.max(), f"'max' slider value greater than maximal value in {k}"
        if v.get('null') == False:
            assert (arr.isna().sum() == 0), f"'null' was defined False for feature `{k}`, but column has NaN values"
...
def _calculate_step_size(arr: pd.Series, decimals: int):
    ...
    return np.max([round(step_size, decimals), round(0.1, decimals)]).astype(arr.dtype).item()
...
def _get_sliders_params(arr, col_dict={}):
    ...
    return {
        'type': 'continuous',
        'min': col_dict.get('min', _calculate_min_max_value(arr, func='min', decimals=decimals)),
        'max': col_dict.get('max', _calculate_min_max_value(arr, func='max', decimals=decimals)),
